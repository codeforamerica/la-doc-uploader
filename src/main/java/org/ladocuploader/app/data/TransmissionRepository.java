...
public interface TransmissionRepository extends JpaRepository<Transmission, UUID> {
    @Query(value = "SELECT s FROM Submission s JOIN Transmission t ON t.submission = s WHERE s.submittedAt IS NOT NULL AND t.time_sent IS NULL ORDER BY s.updatedAt ASC ")
